/* CircularGallery — from React Bits, adapted for SECANT.
   Key modification: added `onItemClick(index)` callback.
   When the user taps/clicks without dragging, the closest centered item
   index is fired to the parent so it can navigate to that category.       */
...
class App {
  constructor(container, { items, onItemClick, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
  ...
  onTouchMove(e) {
    ...
    if (Math.abs(x - this.clickStartX) > 8) this.isClick = false;
  ...
  onTouchUp() {
    ...
    if (this.isClick && this.onItemClick) {
      const idx = this.getCenteredItemIndex();
...
export default function CircularGallery({ items, onItemClick, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px DM Sans', scrollSpeed = 2, scrollEase = 0.05 }) {
  const containerRef = useRef(null);
  useEffect(() => {
    if (!containerRef.current) return;
    const app = new App(containerRef.current, { items, onItemClick, bend, textColor, borderRadius, font, scrollSpeed, scrollEase });
    return () => app.destroy();
  }, [items, onItemClick, bend, textColor, borderRadius, font, scrollSpeed, scrollEase]);
