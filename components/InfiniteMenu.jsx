/* InfiniteMenu — from React Bits, adapted for SECANT.
   Circular disc tiles (DiscGeometry) — smooth SDF edges, no border artefacts.
   Fixes over vanilla source:
     • powerPreference:'high-performance' for smooth 60fps
     • Proper RAF cancellation via this.rafId + stop()
     • onItemClick prop for in-app navigation (no window.open for internal routes)
     • SECANT-branded CSS overlay                                                  */

import { useEffect, useRef, useState } from 'react';
import { mat4, quat, vec2, vec3 } from 'gl-matrix';
import './InfiniteMenu.css';

/* ── Shaders (original from React Bits) ─────────────────────────────────── */
...
/* DiscGeometry — circular tiles, smooth SDF edges, no border artefacts */
...
function createProgram(gl,srcs,tfv,attrs){
  ...
  if(attrs)for(const a in attrs)gl.bindAttribLocation(p,attrs[a],a);
  gl.linkProgram(p);
  if(gl.getProgramParameter(p,gl.LINK_STATUS))return p;
  console.error(gl.getProgramInfoLog(p));gl.deleteProgram(p);return null;
}
function makeVertexArray(gl,blns,indices){
  const va=gl.createVertexArray();gl.bindVertexArray(va);
  for(const[b,l,n]of blns){if(l===-1)continue;gl.bindBuffer(gl.ARRAY_BUFFER,b);gl.enableVertexAttribArray(l);gl.vertexAttribPointer(l,n,gl.FLOAT,false,0,0);}
...
function makeTexture(gl,min,mag,wS,wT){
...
class InfiniteGridMenu {
  ...
  #init(onInit){
    /* powerPreference:'high-performance' → discrete GPU on laptops for smooth 60fps */
    ...
    /* DISC geometry — smooth circular tiles, no border artefacts */
  ...
  #initTexture(){
    ...
    Promise.all(this.items.map(item=>new Promise(res=>{const img=new Image();img.crossOrigin='anonymous';img.onload=()=>res(img);img.src=item.image;}))).then(imgs=>{
  ...
  #render(){
    ...
    /* alpha:true — transparent canvas blends with dark page background, no border artefacts */
...
export default function InfiniteMenu({ items=[], scale=1.0, onItemClick }) {
  ...
  const sketchRef   = useRef(null);          /* stores the InfiniteGridMenu instance */
  ...
  useEffect(()=>{
    ...
    return()=>{
      sketch.stop();            /* cancel RAF — fixes framerate leak on unmount */
      ...
      {activeItem&&(
        <>
          ...
          <div onClick={handleButtonClick} className={`action-button ${isMoving?'inactive':'active'}`}>
            <span className="action-button-text">Open</span>
